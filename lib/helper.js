...
var serializeBSON = require('./bson');
...
exports.buildAuthMessage = function (message, user, passwd, isBigEndian) {
  ...
  var authBytes = serializeBSON(auth, isBigEndian);
...
exports.buildQueryRequest = function (message, isBigEndian) {
  ...
  var query = serializeBSON(message.Matcher, isBigEndian);
  debug('query: %j', query);
  var fieldSelector = serializeBSON(message.Selector, isBigEndian);
  debug('fieldSelector: %j', fieldSelector);
  var orderBy = serializeBSON(message.OrderBy, isBigEndian);
  debug('orderBy: %j', orderBy);
  var hint = serializeBSON(message.Hint, isBigEndian);
  debug('hint: %j', hint);
...
exports.buildInsertRequest = function (message, isBigEndian) {
  ...
  var insertor = serializeBSON(message.Insertor, isBigEndian);
  var nodeID = message.NodeID;
...
exports.buildDeleteRequest = function (message, isBigEndian) {
  ...
  var matcher = serializeBSON(message.Matcher, isBigEndian);
  var hint = serializeBSON(message.Hint, isBigEndian);
...
exports.buildTransactionRequest = function (opCode, requestID, isBigEndian) {
  var messageLength = padLength(MESSAGE_HEADER_LENGTH, 4);
...
exports.buildUpdateRequest = function (message, isBigEndian) {
  ...
  var matcher = serializeBSON(message.Matcher, isBigEndian);
  var hint = serializeBSON(message.Hint, isBigEndian);
  var modifier = serializeBSON(message.Modifier, isBigEndian);
...
exports.appendInsertMessage = function (buff, append, isBigEndian) {
  ...
  var insertor = serializeBSON(append, isBigEndian);
...
exports.buildOpenLobRequest = function (message, isBigEndian) {
  ...
  var flags = message.Flags;
  var contextID = message.ContextIDList[0];
  var bLob = serializeBSON(message.Matcher, isBigEndian);
...
exports.buildWriteLobRequest = function (message, buff, isBigEndian) {
  ...
  var buf = new XBuffer(MESSAGE_OPLOB_LENGTH - MESSAGE_HEADER_LENGTH + MESSAGE_LOBTUPLE_LENGTH, isBigEndian);
  buf.writeInt32(version, 0); // 4 bytes
  buf.writeInt16(w, 4); // 2 bytes
