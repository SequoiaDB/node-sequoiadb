...
Connection.prototype.connectionGone = function (exit) {
  this.conn = null;
  // console.log("connection closed by %s", exit);
};

var Item = function (buff, message, state, callback) {
  this.buff = buff;
...
Connection.prototype._send = function (buff, message, callback, state) {
...
Connection.prototype.auth = function (callback) {
  ...
  this.send(buff, message, callback || function (err) {
    debug('auth done');
  });
};
