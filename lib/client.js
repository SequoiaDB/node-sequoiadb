...
var Client = function (port, host, opts) {
  ...
  Object.defineProperty(this, 'isBigEndian', {
    value: this.pool.isBigEndian,
    enumerable: false
  });
};
util.inherits(Client, EventEmitter);

Client.prototype._send = function (buff, message, callback, state) {
  debug('send message is %j', message);
  debug('buff is %j', buff);
  var item = new Item(buff, message, state, callback);
  if (this.longTrace) {
...
Client.prototype.send = function (buff, message, callback) {
  this._send(buff, message, callback, "Request");
};

Client.prototype.sendLob = function (buff, message, callback) {
  this._send(buff, message, callback, "LobRequest");
};

Client.prototype.isValid = function (callback) {
  if (!this.pool) {
    callback(null, false);
    return;
  }
...
Client.prototype.disconnect = function (callback) {
  ...
  this.pool.disconnect(buff, callback);
  this.pool = null;
};
 ...
 */
Client.prototype.setSessionAttr = function (options, callback) {
...
Client.prototype.getList = function (type, matcher, selector, orderBy, callback) {
 ...
 */
Client.prototype.createCollectionSpace = function (name, options, callback) {
  var matcher = {};
  matcher[constants.FIELD_NAME] = name;

  // createCollectionSpace(name, callback);
  if (typeof options === 'function') {
    callback = options;
    matcher[constants.FIELD_PAGESIZE] = constants.SDB_PAGESIZE_DEFAULT;
  } else if (typeof options === 'object') {
    // createCollectionSpace(name, options, callback);
    util._extend(matcher, options);
  } else {
    // createCollectionSpace(name, pageSize, callback);
 ...
 * @param {String} name collection space name
 */
Client.prototype.dropCollectionSpace = function (name, callback) {
 ...
 * @param {String} name collection space name
 */
Client.prototype.isCollectionSpaceExist = function (name, callback) {
 ...
 * @param {String} name collection space name
 */
Client.prototype.getCollectionSpace = function (name, callback) {
  var that = this;
  this.isCollectionSpaceExist(name, function (err, exist) {
    if (err || !exist) {
      return callback(new SDBError("SDB_DMS_CS_NOTEXIST"));
    }
    callback(null, new CollectionSpace(that, name));
  });
};

Client.prototype.getCollectionSpaces = function (callback) {
 ...
 */
Client.prototype.createProcedure = function (code, callback) {
  if (!code) {
    throw new SDBError("SDB_INVALIDARG");
  }
 ...
 */
Client.prototype.removeProcedure = function (name, callback) {
  if (!name) {
 ...
 */
Client.prototype.getProcedures = function (matcher, callback) {
 ...
 */
Client.prototype.evalJS = function (code, callback) {
  if (!code){
    throw new SDBError("SDB_INVALIDARG");
  }
  ...
  this.sendAdminCommand2(command, matcher, null, null, null,
                         Long.ZERO, Long.ZERO, -1, function (err, response) {
    if (err) {
      return callback(err);
    }
 ...
 */
Client.prototype.exec = function (sql, callback) {
  ...
  this.send(buff, message, function (err, response) {
    if (err) {
      if (err.flags === constants.SDB_DMS_EOC) {
        ...
        callback(new SDBError(err.flags, sql));
      }
      return;
    }
 ...
 */
Client.prototype.execUpdate = function (sql, callback) {
  var message = new Message(constants.Operation.OP_SQL);
  message.RequestID = Long.ZERO; // getNextRequstID();
  message.NodeID = constants.ZERO_NODEID;
  var buff = helper.buildSQLMessage(message, sql, this.isBigEndian);
 ...
 */
Client.prototype.getCollections = function (callback) {
 ...
 */
Client.prototype.getCollectionNames = function (callback) {
 ...
 * @return A list of all collecion space names
 */
Client.prototype.getCollectionSpaceNames = function (callback) {
 ...
 * @return A list of all collecion space names
 */
Client.prototype.getStorageUnits = function (callback) {
...
Client.prototype.createReplicaGroup = function (name, callback) {
  if (!name) {
    throw new SDBError('SDB_INVALIDARG');
  }
 ...
 */
Client.prototype.backupOffline = function (options, callback) {
  if (!options) {
    throw new SDBError("SDB_INVALIDARG");
  }
 ...
 */
Client.prototype.getBackups = function (options, matcher, selector, orderBy, callback) {
  if (!options) {
    throw new SDBError("SDB_INVALIDARG");
  }
 ...
 */
Client.prototype.removeBackup = function (matcher, callback) {
  if (!matcher) {
    throw new SDBError("SDB_INVALIDARG");
  }
