...
Collection.prototype.split = function (source, dest, splitCondition, splitEndCondition, callback) {
  ...
  this.conn.sendAdminCommand(command, matcher, {}, {}, {}, -1, -1, 0, callbackWrap(callback));
};
...
*/
Collection.prototype.updateByQuery = function (query, callback) {
  this._update(0, query.Matcher, query.Modifier, query.Hint, callback);
};
...
*  \note It won't work to update the "ShardingKey" field, but the other fields take effect
*/
Collection.prototype.update = function (matcher, modifier, hint, callback) {
  this._update(0, matcher, modifier, hint, callback);
};
...
Collection.prototype.upsert = function (matcher, modifier, hint, callback) {
  this._update(constants.FLG_UPDATE_UPSERT, matcher, modifier, hint, callback);
};
...
Collection.prototype.getIndexes = function (callback) {
  ...
  this.conn.sendAdminCommand2(command, {}, {}, {}, hint, -1, -1, 0, callbackWrapForCursor(this, callback));
};
...
Collection.prototype.getIndex = function (name, callback) {
  ...
  this.conn.sendAdminCommand2(command, matcher, {}, {}, hint, -1, -1, 0, callbackWrapForCursor(this, callback));
};
...
Collection.prototype.dropIndex = function (name, callback) {
  ...
  matcher[constants.FIELD_COLLECTION] = this.collectionFullName;
  matcher[constants.FIELD_INDEX] = index;
  this.sendAdminCommand2(command, matcher, {}, {}, {}, -1, -1, 0, callbackWrap(callback));
};
...
Collection.prototype.aggregate = function (list, callback) {
  ...
  for (var i = 1; i < list.length; i++) {
    buff = helper.appendAggrMsg(buff, list[i], this.conn.isBigEndian);
  }

  this.conn.send(buff, message, callbackWrap(this, callback));
};
...
Collection.prototype.getQueryMeta = function (matcher, orderBy, hint,
    ...
    this.sendAdminCommand2(command, matcher, hint, orderBy,
...
Collection.prototype.getLobs = function (callback) {
  ...
  matcher[constants.FIELD_COLLECTION] = this.collectionFullName;
  // run command
  this.conn.sendAdminCommand2(command, matcher, {}, {}, {}, 0, -1, 0, callbackWrapForCursor(this, callback));
};
...
Collection.prototype.removeLob = function (id, callback) {
  ...
  var buff = helper.buildRemoveLobRequest(message, this.conn.isBigEndian);
  this.conn.send(buff, message, callbackWrap(callback));
};

Collection.prototype._update = function (flag, matcher, modifier, hint, callback) {
  if (!modifier) {
    throw new Error("SDB_INVALIDARG");
  }
...
Collection.prototype.getMoreCommand = function (response, callback) {
  ...
  var getMore = function (check) {
    ...
    var buff = helper.buildGetMoreRequest(message, this.conn.isBigEndian);
    this.conn.send(buff, message, function (err, response) {
      if (!err) {
