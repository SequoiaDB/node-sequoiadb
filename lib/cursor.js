...
Cursor.prototype.next = function (callback) {
  debug('next');
  if (this.isClosed) {
    return callback(new Error("SDB_DMS_CONTEXT_IS_CLOSE"));
  }
  ...
  if ((this.index === -1) && this.hasMore && !this.list) {
    this.readNextBuffer(function (err) {
      if (err) {
        return callback(err);
      }
      handle();
...
Cursor.prototype.readNextBuffer = function (callback) {
  if (!this.conn) {
    return callback(new Error("SDB_NOT_CONNECTED"));
  }
  debug('read next buffer');

  // compare to -1
  if (Long.equals(this.contextId, Long.NEG_ONE)) {
    this.hasMore = false;
    this.index = -1;
    this.collection = null;
    this.list = null;
...
Cursor.prototype.killCursor = function (callback) {
  if (!this.conn || Long.equals(this.contextId, Long.NEG_ONE)) {
    return callback(null);
  }
  var that = this;
  this.conn.isClosed(function (err, isClosed) {
    if (err) {
      return callback(err);
    }
