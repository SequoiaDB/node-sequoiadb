...
var Cursor = function (response, client, collection) {
  this.index = -1;
  // 循环引用
  this.client = client;
...
Cursor.prototype._readNextBuffer = function (callback) {
  ...
  var buff = helper.buildGetMoreRequest(this.message, this.client.isBigEndian);
  var that = this;
  this.client.send(buff, this.message, function (err, response) {
    if (!err) {
...
Cursor.prototype._killCursor = function (callback) {
  ...
  var buff = helper.buildKillCursorMessage(message, that.client.isBigEndian);
