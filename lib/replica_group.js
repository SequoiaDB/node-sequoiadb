...
var ReplicaGroup = function (client, name, groupId) {
...
ReplicaGroup.prototype.getDetail = function (callback) {
  ...
  this.client.getList(constants.SDB_LIST_GROUPS, matcher, {}, {}, function (err, cursor) {
    if (err) {
      return callback(err);
    }
    if (cursor) {
      cursor.next(function (err, detail) {
        ...
        if (detail) {
          ...
          callback(new SDBError('SDB_CLS_GRP_NOT_EXIST'));
        }
      });
    } else {
      callback(new SDBError('SDB_SYS'));
    }
  });
};
...
ReplicaGroup.prototype.createNode = function (hostname, port, dbpath, map, callback) {
  ...
  var that = this;
  this.client.sendAdminCommand(command, matcher, {}, {}, {}, function (err) {
    if (err) {
      return callback(err);
    }
...
ReplicaGroup.prototype.removeNode = function (hostname, port, configure, callback) {
  ...
  this.client.sendAdminCommand(command, config, {}, {}, {}, function (err) {
    callback(err);
  });
};
...
/**
 * Get the slave node of current group
 * @return The fitted node or null
 */
...
ReplicaGroup.prototype.stopStart = function (start, callback) {
  ...
  this.client.sendAdminCommand(command, matcher, {}, {}, {}, function (err, response) {
    callback(null, !err);
  });
};
