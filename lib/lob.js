...
*/
Lob.prototype.read = function (len, callback) {
  if (!this.isOpen) {
...
Lob.prototype.write = function (buff, callback) {
  ...
  var write = function () {
    ...
    that._write(_buff, function (err) {
      if (err) {
        return callback(err);
      }
 ...
 */
Lob.prototype._write = function (buff, callback) {
  var message = new Message();
  // MsgHeader
...
Lob.prototype._read = function (len, callback) {
  ...
  // send msg
  this.conn.sendLob(bytes, message, function (err, response) {
    if (!err) {
      var buff = response.LobBuff;
      that.readOffset += buff.length;
